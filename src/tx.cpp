...
// 0 = simulated telemetry, 1 = real BME280 telemetry
#define USE_REAL_BME 1
...
static void lora_send_packet(const uint8_t *data, uint8_t len)
{
  ...
  // Send telemetry frame to serial port (USB CDC) for ground station monitoring
  noInterrupts();
...
static void telemetry_flush_packet()
{
  ...
  // Ha semmi sem változott, akkor is küldünk: folyamatos kulcs/delta frame-ek szükségesek.
...
void setup()
{
  ...
  if (!bme_begin())
  {
    // BME280 init failed; keep waiting to avoid sending garbage.
  }
  g_sensor_ready = false;
  g_sys_state = SYS_WAIT_SENSORS;
#else
